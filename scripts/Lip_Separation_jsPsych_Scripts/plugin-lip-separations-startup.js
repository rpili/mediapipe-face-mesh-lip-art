var jsPsychLipSeparationsFacemeshStartup = (function (jspsych) {
  'use strict';

  const info = {
      name: "facemesh-startup",
      parameters: {
          /** Begin drawing selected elements of the FaceMesh tesselation from the tracked face to the canvas, refreshing every 50 ms. **/
          stimulus: {
  ...
  /**
   * **Lip Separations via Facemesh Startup**
   *
   * jsPsych plugin for initializing face-tracking via FaceMesh, for use with the Lip Separations via FaceMesh extension.
   *
   ...
   * 
   */
  class LipSeparationsFacemeshStartupPlugin {
      constructor(jsPsych) {
      ...
      trial(display_element, trial) {
          ...
          if (Array.isArray(trial.button_html)) {
              if (trial.button_html.length == trial.choices.length) {
                  ...
                  console.error("Error in lip-separations-startup plugin. The length of the button_html array does not equal the length of the choices array");
              }
          }
          else {
  ...
  LipSeparationsFacemeshStartupPlugin.info = info;

  return LipSeparationsFacemeshStartupPlugin;
