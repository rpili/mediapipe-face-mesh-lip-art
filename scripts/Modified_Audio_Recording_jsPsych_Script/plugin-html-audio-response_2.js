var jsPsychHtmlAudioResponse2 = (function (jspsych) {
   ...
   * @author Peter A. Krause lightly modified this from the existing plugin by Josh de Leeuw.
   * There is a slight change in functionality that allows three successive stimuli to be presented, each with the own durations, while the microphone is continuously recording.
   */
  class HtmlAudioResponse2Plugin {
      ...
      showDisplay2(display_element, trial) {
        const ro = new ResizeObserver((entries, observer) => {
            this.stimulus_2_start_time = performance.now();
            observer.unobserve(display_element);
            //observer.disconnect();
        });
        ro.observe(display_element);
        let html = `<div id="jspsych-html-audio-response-stimulus_2">${trial.stimulus_2}</div>`;
        display_element.innerHTML = html;
    }
    hideStimulus2(display_element) {
        const el = display_element.querySelector("#jspsych-html-audio-response-stimulus_2");
        if (el) {
            el.style.visibility = "hidden";
        }
    }
    showDisplay3(display_element, trial) {
        const ro = new ResizeObserver((entries, observer) => {
            this.stimulus_3_start_time = performance.now();
            observer.unobserve(display_element);
            //observer.disconnect();
        });
        ro.observe(display_element);
        let html = `<div id="jspsych-html-audio-response-stimulus">${trial.stimulus_3}</div>`;
        display_element.innerHTML = html;
    }
    hideStimulus3(display_element, trial) {
        const ro = new ResizeObserver((entries, observer) => {
            this.blank_screen_start_time = performance.now();
            observer.unobserve(display_element);
            //observer.disconnect();
        });
        ro.observe(display_element);
        let html = `<div id="jspsych-html-audio-response-stimulus"> </div>`;
        display_element.innerHTML = html;
    }
          ...
          this.start_event_handler = (e) => {
              ...
              if (trial.stimulus_duration !== null) {
                  this.jsPsych.pluginAPI.setTimeout(() => {
                      this.showDisplay2(display_element, trial);
                  }, trial.stimulus_duration);
              }
              if (trial.stimulus_duration_2 !== null) {
                this.jsPsych.pluginAPI.setTimeout(() => {
                    this.showDisplay3(display_element, trial);
                }, trial.stimulus_duration_2 + trial.stimulus_duration);
            }
              if (trial.stimulus_duration_3 !== null) {
                  this.jsPsych.pluginAPI.setTimeout(() => {
                      this.hideStimulus3(display_element);
                  }, trial.stimulus_duration_3 + trial.stimulus_duration_2 + trial.stimulus_duration);
              }
